...
export default function AdminConfig() {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      setNewValue('');
      setSuccess(`Added "${newValue.trim()}" to ${activeTab}.`);
      loadAll();
    } catch (err) {
      setError(err.response?.data?.error || 'Failed to add config value.');
    } finally {
      setAddLoading(false);
    }
  };
  ...
  const handleDelete = async (config) => {
    ...
    try {
      await API.config.remove(id);
      setSuccess(`Deleted "${val}".`);
      loadAll();
    } catch (err) {
  ...
  const handleSeed = async () => {
    ...
    try {
      await API.config.seed();
      setSuccess('Default values seeded successfully.');
      loadAll();
    } catch (err) {
      setError(err.response?.data?.error || 'Failed to seed defaults.');
    }
  };
    ...
    <div>
      <div className="page-header">
        <div className="flex-between">
          <div>
            <h1>Configuration</h1>
            <p>Manage dropdown values and system settings</p>
          </div>
        ...
        <form onSubmit={handleAdd} className="form-row" style={{ marginBottom: '20px' }}>
          <div className="form-group" style={{ flex: 1 }}>
            ...
            <input
              ...
              placeholder={`Enter ${activeTab.replace('_', ' ')} value`}
            />
          </div>
                ...
                {currentConfigs.map((config, i) => {
                  ...
                  return (
                    <tr key={config.id || config._id || i}>
                      ...
                      <td>{val}</td>
                      <td>
                        <button className="btn btn-danger btn-sm" onClick={() => handleDelete(config)}>Delete</button>
                      </td>
                    </tr>
                  );
                })}
