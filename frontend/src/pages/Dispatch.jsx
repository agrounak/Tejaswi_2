...
export default function Dispatch() {
  ...
  const handleStartDispatch = async (e) => {
    ...
    try {
      ...
      setStep(2); setSuccess('Dispatch started.'); loadActiveDispatches()
    } catch (err) {
  ...
  const handleScan = async (e) => {
    ...
    try {
      ...
      setScanInput(''); setSuccess('Item scanned successfully.')
    } catch (err) {
  ...
  const handleRemoveItem = async (itemId) => {
    ...
    } catch (err) { setError(err.response?.data?.error || 'Failed to remove item.') }
  }
  ...
  const handleUpdateDetails = async (e) => {
    ...
    try {
      ...
      const res = await API.dispatch.updateDetails(id, {
        vehicle_number: vehicleNumber, driver_name: driverName, driver_phone: driverPhone,
      })
      setCurrentDispatch(res.data?.dispatch || res.data)
      setStep(4); setSuccess('Details updated.')
    } catch (err) { setError(err.response?.data?.error || 'Failed to update details.') }
  }
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      setCurrentDispatch(null); setStep(1); setClientName(''); loadActiveDispatches()
    } catch (err) { setError(err.response?.data?.error || 'Failed to finalize dispatch.') }
    finally { setLoading(false) }
  }
    ...
    <div>
          ...
          {activeDispatches.length > 0 && (
            <div className="card">
              ...
              <div className="table-container">
                <table>
                  <thead><tr><th>Dispatch #</th><th>Client</th><th>Date</th><th>Items</th><th>Action</th></tr></thead>
                  <tbody>
                    {activeDispatches.map((d, i) => (
                      <tr key={d.id || d._id || i}>
                        ...
                        <td><button className="btn btn-primary btn-sm" onClick={() => handleResumeDispatch(d)}>Resume</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {step === 2 && currentDispatch && (
        <div className="card">
          ...
          <div className="stats-grid" style={{ marginBottom: '16px' }}>
            <div className="stat-card blue"><div className="stat-label">Total Items</div><div className="stat-value">{totalItems}</div></div>
            <div className="stat-card green"><div className="stat-label">Total Weight</div><div className="stat-value">{totalWeight.toFixed(1)} kg</div></div>
          </div>
          <form onSubmit={handleScan} className="form-row" style={{ marginBottom: '16px' }}>
            <div className="form-group" style={{ flex: 1 }}>
              <label>Product Number</label>
              <input type="text" value={scanInput} onChange={(e) => setScanInput(e.target.value)} placeholder="Scan or enter product number" autoFocus />
            </div>
          ...
          {dispatchItems.length > 0 && (
            <div className="table-container">
              <table>
                <thead><tr><th>#</th><th>Product #</th><th>Quality</th><th>GSM</th><th>Weight</th><th>Action</th></tr></thead>
                <tbody>
      ...
      {step === 3 && currentDispatch && (
        <div className="card">
          ...
          <form onSubmit={handleUpdateDetails}>
            <div className="form-grid">
              <div className="form-group"><label>Vehicle Number</label><input type="text" value={vehicleNumber} onChange={(e) => setVehicleNumber(e.target.value)} placeholder="e.g., MH12AB1234" /></div>
              <div className="form-group"><label>Driver Name</label><input type="text" value={driverName} onChange={(e) => setDriverName(e.target.value)} placeholder="Driver name" /></div>
              <div className="form-group"><label>Driver Phone</label><input type="text" value={driverPhone} onChange={(e) => setDriverPhone(e.target.value)} placeholder="Phone number" /></div>
            </div>
            <div style={{ marginTop: '16px' }} className="btn-group">
      ...
      {step === 4 && currentDispatch && (
        <div className="card">
          ...
          <div className="stats-grid" style={{ marginBottom: '16px' }}>
            <div className="stat-card blue"><div className="stat-label">Client</div><div className="stat-value" style={{ fontSize: '18px' }}>{currentDispatch.client_name}</div></div>
            <div className="stat-card green"><div className="stat-label">Total Items</div><div className="stat-value">{totalItems}</div></div>
            <div className="stat-card orange"><div className="stat-label">Total Weight</div><div className="stat-value">{totalWeight.toFixed(1)} kg</div></div>
          </div>
